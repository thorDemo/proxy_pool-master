...
class GetFreeProxy(object):
    ...
    def payProxyFirst():
        proxies = []
        url = 'http://47.96.139.87:8081/Index-generate_api_url.html?packid=7&fa=5&qty=20&port=1&format=json&ss=5&css=&ipport=1&et=1&pro=&city='
        html = requests.get(url).content.decode('utf-8')
        # print(html)
        result = json.loads(html)
        for x in range(0, len(result['data'])):
            proxies.append(result['data'][x]['IP'])
        for proxy in proxies:
            yield proxy
...
if __name__ == '__main__':
    ...
    CheckProxy.checkGetProxyFunc(GetFreeProxy.payProxyFirst)
    # CheckProxy.checkGetProxyFunc(GetFreeProxy.freeProxySecond)


    # gg = GetFreeProxy()

    # test_batch(gg.payProxyFirst())
